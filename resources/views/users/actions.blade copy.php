@if ($model->trashed())
    <x-utils.link
        :href="route('auth.users.restore', $model)"
    ...
    @if (config('boilerplate.access.users.permanently_delete'))
        <x-utils.delete-button :href="route('auth.users.permanently-delete', $model)" permission="access.users.permanently-delete" :text="__('Permanently Delete')" />
    @endif
@else
    <x-utils.view-button :href="route('auth.users.show', $model)" permission="access.users.read" />
    {{--<x-utils.edit-button :href="route('auth.users.edit', $model)" permission="access.users.update" />--}}

    @if (! $model->isActive())
        <x-utils.link
            :href="route('auth.users.mark', [$model, 1])"
    ...
    @if ($model->id !== 1 && $model->id !== auth()->id())
        <x-utils.delete-button :href="route('auth.users.destroy', $model)" permission="access.users.delete" />
    @endif
    ...
    {{--            <a class="dropdown-item" href="#">Change Password</a>--}}
                @if ($model->id !== 1 && $model->id !== auth()->id())
                    <x-utils.link
                        :href="route('auth.users.mark', [$model, 0])"
                        class="dropdown-item"
                        :text="__('Deactivate')"
                        name="confirm-item"
