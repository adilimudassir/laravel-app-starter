@if ($model->trashed())
    <x-utils.link
        :href="route('backend.auth.users.restore', $model)"
    ...
    @if (config('boilerplate.access.users.permanently_delete'))
        <x-utils.delete-button :href="route('backend.auth.users.permanently-delete', $model)" permission="access.users.permanently-delete" :text="__('Permanently Delete')" />
    @endif
@else
    <x-utils.view-button :href="route('backend.auth.users.show', $model)" permission="access.users.read" />
    {{--<x-utils.edit-button :href="route('backend.auth.users.edit', $model)" permission="access.users.update" />--}}
    ...
    @if ($model->id !== 1 && $model->id !== auth()->id())
        <x-utils.delete-button :href="route('backend.auth.users.destroy', $model)" permission="access.users.delete" />
    @endif
